...
const TrophyDetails: React.FC = () => {
	...
	const [simeller, setsimeller] = useState<BoxType[]>([]);
	...
	if (!item)
		return (
			<div className='grid grid-cols-2 m-auto pt-32 max-w-screen-lg'>
				<LoadCard />
				<LoadCard />
			</div>
		);
		...
		<section className='max-w-screen-lg m-auto py-28 container space-y-10'>
			...
			<div className='relative gap-10 rounded-tr-3xl shadow-md hover:shadow-xl transition-shadow overflow-hidden rounded-lg grid grid-cols-2 m-auto duration-300'>
				...
				<div className='relative  bg-white p-14 text-start'>
					...
					<span className='absolute -right-px -top-px rounded-bl-3xl z-10 rounded-tr-3xl bg-gradient-to-r from-rose-600 to-pink-500 px-6 py-4 font-medium uppercase tracking-widest text-white'>
						{item.category}
					</span>
			...
			{simeller.length > 0 ? (
				<div>
					<h1 className='pb-3 pt-20 text-gray-500 text-4xl font-bold'>
						{' '}
						Similer Products
					</h1>
					<div className='grid pt-10 grid-cols-3 gap-4'>
						{simeller.map((simItem) => (
							<Box
								id={simItem.id}
								title={simItem.title}
								price={simItem.price}
								...
								ratingCount={simItem.rating.count.toString()}
							/>
						))}
					</div>
				</div>
